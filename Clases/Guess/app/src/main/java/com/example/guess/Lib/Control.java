...
public class Control {
    ...
    public static int menuGenerator(String tittle, String[] sentence) {
        ...
        int option = Integer.parseInt(Lib.Util.myInput.nextLine());
    ...
    public static GregorianCalendar randBirth(int min, int max) {
        ...
        int year = Lib.Util.randBetween(min, max);
        // we set the our own year to one gc YEAR
        gc.set(gc.YEAR, year);
        // get a random day of the year
        int dayOfYear = Lib.Util.randBetween(1, gc.getActualMaximum(gc.DAY_OF_YEAR));
    ...
    public static boolean one(String message, char[] validate) {
        ...
        char c = Lib.Util.myInput.nextLine().toLowerCase().charAt(index);
    ...
    public static <T extends Enum<T>> Enum<T> selectEnum(T[] values) {
        ...
        do {
            ...
            int choice = Lib.Util.validateInt(message, 0, values.length);
            for (int i = 0; i < values.length; i++) {
                if (i == choice) {
                    return values[i];
                } else {
     ...
     * 
     * @param <T>
     * @param option you must to select
    ...
    public static <T> T checkOptions(String[] option) {
        do {
            ...
            String usr = Lib.Util.myInput.nextLine();
