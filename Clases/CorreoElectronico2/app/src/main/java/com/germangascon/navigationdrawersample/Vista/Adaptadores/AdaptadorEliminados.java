...
public class AdaptadorEliminados extends RecyclerView.Adapter<AdaptadorEliminados.HolderCorreoRecibidos> {

    private final Context context;
    private final IOnCorreoSeleccionado iOnCorreoSeleccionado;
    private final HashMap<Email, Contacto> hashMap;

    public AdaptadorEliminados(Context context, Cuenta cuenta, IOnCorreoSeleccionado iOnCorreoSeleccionado) {
        this.context = context;
        this.iOnCorreoSeleccionado = iOnCorreoSeleccionado;
        CorreoLogica correoLogica = new CorreoLogica(cuenta);
        hashMap = correoLogica.getCorreosEliminados();
    }

    @NonNull
    @Override
    public HolderCorreoRecibidos onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        final View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.fragmento_recibidos,parent,false);
    ...
    public class HolderCorreoRecibidos extends RecyclerView.ViewHolder implements View.OnClickListener{
        ...
        public HolderCorreoRecibidos(@NonNull View itemView) {
            super(itemView);
            this.imageView = itemView.findViewById(R.id.imageRecibido);
            this.tvNombre = itemView.findViewById(R.id.tvNombreRecibido);
            this.tvTema = itemView.findViewById(R.id.tvAsuntoRecibido);
            this.tvDescripcion = itemView.findViewById(R.id.tvTextoRecibido);
            this.tvFecha = itemView.findViewById(R.id.tvFechaRecibido);
